...
function moveToOriginalPosition() { // 선택된 top5 아래의 하나의 이미지를 제자리로 이동시킴
    ...
    var current = $(imgElement).prev().text();
	$("#top5>span").eq(current-1).remove();
	...
	$("#cds>img").each(
		function() {
			if(Number($(imgElement).attr("id")) < Number($(this).attr("id"))) {
				$(imgElement).insertBefore(this);
                isLast = 0;
